...
// FIX: Changed from 'extends EventEmitter' to composition to resolve typing issues with event handlers.
export class GenAILiveClient {
  ...
  // FIX: Use a composed EventEmitter instance.
  ...
  public send(parts: Part | Part[], turnComplete: boolean = true) {
    if (this._status !== 'connected' || !this.session) {
      // FIX: Changed to use the composed emitter instance.
  ...
  protected onMessage(message: LiveServerMessage) {
    ...
    if (message.serverContent) {
      ...
      const audioParts = modelTurnParts.filter(p =>
        p.inlineData?.mimeType?.startsWith('audio/pcm'),
      );
      const otherParts = difference(modelTurnParts, audioParts);
      ...
      if (otherParts.length > 0 || serverContent.groundingMetadata) {
        const contentPayload = { ...serverContent };
        if (contentPayload.modelTurn) {
          contentPayload.modelTurn.parts = otherParts;
        }

        // FIX: Changed to use the composed emitter instance.
        this.emitter.emit('content', contentPayload);
        this.log(`server.content`, contentPayload);
      }
