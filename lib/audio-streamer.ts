...
export class AudioStreamer {
  ...
  private scheduleNextBuffer() {
    ...
    if (this.audioQueue.length === 0) {
      if (this.isStreamComplete) {
        // Don't set isPlaying to false here. Wait for the `onended` event of the last buffer.
  ...
  complete() {
    this.isStreamComplete = true;
    // If we are not playing and have an empty queue, the stream is already "complete".
    // The onComplete handler should be called to finalize the process.
    if (!this.isPlaying && this.audioQueue.length === 0) {
      // Use a timeout to ensure this runs after the current execution stack.
      // This allows the caller (e.g., use-live-api) to attach the onComplete
      // handler just before calling complete().
      setTimeout(() => {
        if (this.onComplete) {
          this.onComplete();
        }
